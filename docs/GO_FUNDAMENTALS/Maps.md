...
In arrays & slices, you saw how to store values in order. Now, we will ook at a way to store items by a `key` and look them up quickly.
...
First, asusming we already have some words with their definitions in the dictionary, if we search for a word, it should return the definition of it.
...
func TestSearch(t *testing.T) {
	...
	assertStrings(t, Search(dictionary, "test"), "this is just a test")
}
...
We started using the `Dictionary` type, which we have not defined yet. Then called `Search` on the `Dictionary` instance. We did not need to change `assertStrings`.
...
Notice that as we've seen in the pointers and errors section, here in order to assert the error message we first check that the error is not `nil` and then use `.Error()` method to get the string which we can then pass to the assertion.
...
```sh
.\dictionary_test.go:9:13: assignment mismatch: 2 variables but dict.Search returns 1 value
.\dictionary_test.go:15:14: assignment mismatch: 2 variables but dict.Search returns 1 value
FAIL    github.com/sdutill/go-learn-with-tdd/src/go_fundamentals/maps [build failed]
```
...
--- FAIL: TestSearch (0.00s)
    --- FAIL: TestSearch/unknown_word (0.00s)
        dictionary_test.go:19: expeccted to get an error.
FAIL
```
...
We have a great way to search the dictionary. However, we have no way to add new words to out dictionary.
...
In this test, we are utilizing our `Search` functoin to make the validation of the dictionary a little easier.
...
```sh
--- FAIL: TestAdd (0.00s)
    dictionary_test.go:12: should find added word: could not find the word you were looking for
FAIL
```
...
Adding to a map is also simliar to an array. You just need to specify a key and set it equal to a value.
...
A gotcha with maps is that they can be a `nil` value. A `nil` map behaves like an ampty map when reading, but attempts to write to a `nil` map will cause a runtime panic. You can read more about maps [here](https://blog.golang.org/go-maps-in-action).
...
Map will not throw an error if the value already exists. Instead, they will go ahead and overwrite the value with the newly provided value. This can be convenient in practice, but makes our functoin name less than accurate. `Add` should not modify existing values. It should only add new words to our dictionary.
...
```sh
.\dictionary_test.go:11:10: dict.Add(word, definition) (no value) used as value
.\dictionary_test.go:23:10: dict.Add(word, definition) (no value) used as value
.\dictionary_test.go:25:23: undefined: ErrWordExists
FAIL    github.com/sdutill/go-learn-with-tdd/src/go_fundamentals/maps [build failed]
```
...
--- FAIL: TestAdd (0.00s)
    --- FAIL: TestAdd/existing_word (0.00s)
        dictionary_test.go:25: got %!q(<nil>) want "cannot add word because it already exists"
```
...
```sh
.\dictionary_test.go:12:8: dict.Update undefined (type Dictionary has no field or method Update)
FAIL    github.com/sdutill/go-learn-with-tdd/src/go_fundamentals/maps [build failed]
```
...
There is not refactoring we need to do on this since it was a simple change However, we now have the same issue as with `Add`. If we pass in a new word, `Update` will add it to the dictionary.
...
With these changes, we not get a very clear error:
...
This function looks almost identical to `Add` except we switched when we update the `dict` and when we return an error.
...
Next, let's create a functoin to `Delete` a word in the dictionary.
...
Our test created a `Dictionary` with a word and then checks if the word has been removed.
...
```sh
.\dictionary_test.go:9:7: dict.Delete undefined (type Dictionary has no field or method Delete)
FAIL    github.com/sdutill/go-learn-with-tdd/src/go_fundamentals/maps [build failed]
```
...
```sh
.\dictionary.go:56:1: missing return
FAIL    github.com/sdutill/go-learn-with-tdd/src/go_fundamentals/maps [build failed]
```
...
In this section, we covered a lot. We made a full CRUD (Create, Read, Updte, and Delete) API for our dictoinary. Throughout the process we learned how to:
