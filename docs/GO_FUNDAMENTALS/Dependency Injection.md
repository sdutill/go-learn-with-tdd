...
Tghere are _a lot_ of misunderstandings around dependency injection around the programming community. Hopefully, this guide will show you how
...
We want to write a function that greets someone, just like we did in the hello-world chapter but this time we are goig to be testing the _actual printing_.

Just to recap, here is what the function could look like
...
If we do that, we can then change the implementatoin to print to something we control so that we can test it. In "real life" you would inject in something that writes to stdout.
...
Sp we know under the covers we're ultimately using `Writer` to send our greeting somewhere Let's use this existing abstraction to make our code testable and more reusable.
...
FAIL    github.com/sdutill/go-learn-with-tdd/src/go_fundamentals/dependency_injection [build failed]
```
...
_Listen to the compiler and fix the problem._
...
Hello, Chris--- FAIL: TestGreet (0.00s)
    di_test.go:16: got '""' want '"Hello, Chris"'
FAIL
```
...
To demonstrate this, try wiring up the `Greet` function into a Go appliaction where we want to print to stdout.
...
```sh
.\main.go:6:8: cannot use os.Stdout (variable of type *os.File) as *bytes.Buffer value in argument to Greet
```

As discussed earlier, `fmt.Fprintf` allows you to pass in an `ioWriter` which we know both os.Stdout and `bytes.Buffer implement.

If we change our code to use the more general purpose interface we can now use it in both tests and in our appliaction.
...
Run the program and go to [http://localhost:5001]. You'll see your greeting functoin being used.
...
_Motivated by our tests_ we refactored the code so we could control _where_ the data was written by **injectingg a dependency** which allowed us to:

- **Test our code** If you can't test a function _easily_, it's usually because of dependencies hard-wired into a function _or_ global state. If you have a global database connection pool for instance that is used by some kind of service layer, it is going to be difficult to test and they will be slow to run. DI will motivate you to inject in a database dependency (via an interface) which you can then mock out with something you can control in your tests.
- **Separate our concerns**, decoupling _where the data goes_ from _how to generate it._ If you ever fel like a method/function has too many responsibilities (generating data _and_ writing to a db? handling HTTP requests _and_ doing domain level logic?) DI is probably going to be the tool you need.
- **Allow our code to be re-used in different contexts** The first "new" context our code can be used in is inside tests. But further on if someonewants to try something new with your function they can inject their own dependencies.
...
By having some familiarity with the `io.Writer` interface we are able to use `bytes.Buffer` in our test as our `Writer` and then we can use other `Writer`s from the standard library to use our function in a command line app or in web servers.
