...
We'll tackle this by writing a function called `Countdown` which we will then put inside a `main` program so it looks slmething like this:
...
While this is a pretty trivial program, to test it fully we will need as always to take an _iterative, test-driven_ approach.
...
If anything like `buffer` is umfamiliar to you, re-read the previous section.

We know we want our `Countdown` function to write data somewhere and `io.Writer` is the de-facto way capturing that as an interface in Go.

- I `main` we will sewnd to `os.Stdout` so our users see the countdown printed to the terminal.
...
FAIL    github.com/sdutill/go-learn-with-tdd/src/go_fundamentals/mocking [build failed]
```
...
We're using `fmt.Fprintf` which takes an `io.Writer` (like `*bytes.Buffer`) and sends a `string` to it. The test should pass.

## Refactor

We that while `*bytes.Buffer` works, it would be better to use a general purpose interface instead.
...
To complete matters, let's now wire up our functoin into a `main` so we have some working software to reassure ourselves we're making progress.
...
Yes this seems trivial but this approach is what Iwould recommend for any project. **Take a thin slice of functionality and make it work end-to-end, backed by tests.**
...
```sh
--- FAIL: TestCountdown (0.00s)
    countdown_test.go:20: got '"3"' want '"3\n2\n1\nGo!"'
FAIL
```
...
There's not much to refactor other than refactoring some magiv values into names constants.
...
- We have not tested an important property of the function.
...
Let's define our dependency as an interface. This lets us then use a _real_ Sleeper in `main` and a _spy sleeper_ in our tests. By using an interface our `Countdown` function is oblivious to this and adds some flexibilty for the caller.
...
I made a design decition that our `Countdown` function would not be responsible for how long the sleep is. This simplifies our code a little for now at least and means a user of our functions can configure that sleepiness however they like.
...
```sh
.\countdown_test.go:12:20: too many arguments in call to Countdown
...
```sh
.\countdown.go:34:12: not enough arguments in call to Countdown
        have (*os.File)
...
This test should pass and no longer take 3 seconds.
...
Our `SpyCountdownOperations` implements both `io.Writer` and `Sleeper`, recording every call into one slice. In this test we're only concerned about the oerder of operations, so just recording the as a list of named operations is sufficient.

We can now add a sub-test suite which verifies our sleeps and prints operate in the order we hope.
...
We are using `duratoin` to configure the time slept and `sleep` as a way to pass in a sleep function. The signature of `sleep` is the same as for `time.Sleep` allowing us to use `time.sleep` in our real implementation and the following spy in our tests:
...
```go
func TeestCountdownSleep(t *testing.T) {
...
There should be nothing new in this test and it is set up very simliar to the previous mock tests.

### Try and run the test

```sh
.\countdown_test.go:56:10: sleeper.Sleep undefined (type ConfigurableSleeper has no field or method Sleep, but does have field sleep)
FAIL    github.com/sdutill/go-learn-with-tdd/src/go_fundamentals/mocking [build failed]
```

You should see a very clear error messge indicating that we do not have a `Sleep` method created on our `ConfigurableSleeper`.
...
```sh
--- FAIL: TestCountdownSleep (0.00s)
    countdown_test.go:59: should have slept for 5s but slept for 0s
FAIL
```
...
Since we are using the `ConfigurableSleeper`, it is now safe to delete the `DefaultSleeper` implementatin. Wrapping up our program and having a more generic Sleeper with arbitrary long countdowns.
...
People normally get in to a bad state when they don't _listen to their tests_ and are `not respecting the refactoring stage.

If your mocking code is becoming complicated or you are having to mock out lots of things to get something, you should _listen_ to that bad feeling and think about your code. Usually it is a sign of
...
This is usually a sign of you testing too much _implementation detail_. Try to make it so your tests are testing _useful behavior_ unless the implementation is really important to how the system runs.

It is sometimes hard to know _what level_ to test exactly but here are some thought processes and rules I try to follow:

- **The definintion of refactoring is that the code changes but the behavior stays the same.** If you have decided to do some refactoring in theory you should be able to make the commit without any test changes. So when writing a test ask yourself:
  - Am I testing the behavior I want, or the implementatoin of details?
...
- Use spies with caution. Spies let you see the insides of the algorithm you are writing which can be very useful but that means a tighter coupling between your test code and the implementatoin. **Be sure you actually care about these details if you're going to spy on them**
...
- a better understandinf of how to mock
...
- When faced with less trivial examples, break the problem down into "thi vertical slices". Try to get to a point where you have _working software backed by tests_ as soon as you can, to avoid getting rabbit holes and taking a "big ang" approach.
...
- **Without mocking important areas of your code will be untested**. In our case we would not be able to test that ou code paused between each print but there are countless other examples. Calling a service that _can_ fail? Wanting to test your system in a particular state? It is vey hard to test these scenarios without mocking.
- Without mocks you may have to set up databases and other third party things just to test simple business rules. You're likely to have slow tests, resulting in **slow feedback loops**.
...
> [Test Double is a generic term for any case whereyou replace a production object for testing purposes.](https://martinfowler.com/bliki/TestDouble.html)
...
In Go 1.23 iterators were introduced. We can use iterators in various ways, in this instance we can make a `countDownFrom` iterator, which will return the numbers to countdown in reverse order.
...
In our case, we don't have keys, just values. Go also provides a convenience type `iterSeq[T]` which is a type alias for `func(func(T) bool)`.
